...
Programmateur programmateurs[] = {
  { "1", "Programmation 1", "Jardin",  "#f59e0b", 32, true, "06:45-08:10,09:00-13:15", true, false, 14 }, //#f59e0b est un code couleur hexadécimal R V B
  { "2", "Programmation 2", "Portail",  "#06b6d4", 33, true, "07:05-09:10,17:08-19:33", true, false, 16 },
  { "3", "Programmation 3", "Relais 3", "#0CE892", 25, true, "", true, false, 17 },  // "" ->aucune plage
  { "4", "Programmation 4", "Relais 4", "#ef4444", 26, true, "23:17-01:52", true, false, 18 }, 
...
<div class="modal-overlay" id="infoOverlay" onclick="if(event.target===this) closeInfo()">
  <div class="modal-box">
    ...
    <!-- 🆕 Vérifie, sans moniteur série, si le dernier redémarrage a bien
         détecté un nouveau firmware et réinitialisé les réglages (NVS) —
         pratique juste après une mise à jour OTA. -->
    <div class="modal-row"><span class="lbl">Réglages réinitialisés</span><span class="val" id="info-reset">---</span></div>
...
if (d.auto && d.restant >= 0) {
  ...
  remainEl.innerText = "";
      ...
      if (p !== gridId) {
        ...
        const info = computeRangeInfo(plages, hmToMin(data.actuelle));
...
document.getElementById('info-build').innerText = formatBuildFR(info.build);
    // 🆕 "Oui" = la structure NVS a été réinitialisée parce que CONFIG_VERSION
    // a changé ; "Non" = les réglages existants ont été conservés.
    // 🎨 innerHTML (et non innerText) pour pouvoir forcer un retour à la
    // ligne avec <br> entre "(précédent :" et la date, sinon ignoré par le HTML.
    document.getElementById('info-reset').innerHTML = info.nvsReset
      ? `Oui ✅ Précédent :<br>${formatBuildFR(info.buildPrecedent)}`
      : "Non <br>réglages conservés";
  } catch (e) {
...
// ============================================================================
// 🆕 CORRECTIF SÉCURITÉ : les routes qui modifient les relais ou les
// programmations demandent une authentification HTTP Basic. Le mot de passe
// réutilise SECRET_OTA_PASSWORD afin de ne pas ajouter un second secret dans
// le fichier principal ; l'identifiant Web est "admin".
bool authentifierWeb(AsyncWebServerRequest *request) {
...
void setup() {
 ...
 // 🚩3️⃣🚨 Attente active de la synchronisation de l'heure NTP
  ...
  server.on("/toggle-mode", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (!authentifierWeb(request)) return; // 🆕 CORRECTIF : protège toute action de modification
